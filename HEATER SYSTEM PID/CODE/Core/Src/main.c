...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if ((HAL_GetTick() - timestap) > 100)
		dir = 0;
	if (GPIO_Pin == GPIO_PIN_1 && dir == 0 || GPIO_Pin == GPIO_PIN_1 && dir == 1)
	{
		encoderCount++;
		PID_Init(&pid, 1.0, 0.1, 0.01, encoderCount);
	...
	if (GPIO_Pin == GPIO_PIN_0 && dir == 0 || GPIO_Pin == GPIO_PIN_0 && dir == 2)
	{
		if (encoderCount > 0)
		{
			encoderCount--;
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		...
		if (status == 1)
		{
			...
			__HAL_TIM_SetCompare(&htim2, TIM_CHANNEL_2, (uint16_t)control_signal);
			updateTime();
		}
		else {
